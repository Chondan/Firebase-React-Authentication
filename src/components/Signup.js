...
export default function Signup() {
	...
	async function handleSubmit(e) {
		...
		try {
			setError('');
			setLoading(true);
			const err = await signup(emailRef.current.value, passwordRef.current.value);
			setLoading(false);
			if (err) return setError(err.message);
			history.push(getFullPath('/'));
	...
	return (
		<>
			...
			<div className="w-100 text-center mt-2">
				Already have an account? <Link to={getFullPath('/login')}>Log In</Link>
			</div>
		</>
	);
}